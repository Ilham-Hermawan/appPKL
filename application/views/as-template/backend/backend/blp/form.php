...
<div class="content-wrapper">
  ...
  <section class="content-header content-header-default">
    ...
    <ol class="breadcrumb">
      ...
      <li>Biaya Legalitas & Perizinan</li>
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->blp_id) ? $id = $data['set']->blp_id : $id = "0";
    ...
    ?>
    <?= form_open_multipart("dashboard/owner/blp/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
      ...
      <div class="box-body">
        ...
        <table style="width:100%" id="table">
          ...
          <tbody>
            ...
            <?php
              $uraian = array(
                'Biaya IPPT [BPN]', 'Biaya Penggabungan Sertifikat', 'Biaya Sertifikat Kavling', 'Biaya PBB Kavling',
                'Biaya IPPT & Site Plan', 'Biaya UKL/UPL [1.8 Hektar]', 'Biaya Rekomendasi Lurah',
                'Biaya Rekomendasi Camat', 'Biaya SK. Akses Jalan & Saluran', 'Biaya SK. Bebas Banjil / Peil Banjir',
                'Biaya SK. PDAM / Uji Lab Kualitas Air', 'Biaya SK. PLN Mensuplay Listrik', 'Biaya SK. IMB Proses',
                'Biaya Advis IMB [Type 36]', 'Biaya Retribusi IMB [Type 36]', 'Biaya Lain-lain'
              );
            ?>
            <?php if(!empty($data['detil'])): ?>
              <?php foreach($data['detil'] as $row): ?>
                <tr>
                  ...
                  <td>
                    <div class="form-group"><input type="text" value="<?= $row->drbl_uraian ?>" class="form-control" name="uraian[]" placeholder="Uraian" autocomplete="off"></div>
                    <input type="hidden" name="drbl_id[]" value="<?= $row->drbl_id ?>">
                  </td>
                  <td><div class="form-group"><input type="text" value="<?= ($row->drbl_volume == 0) ? "" : $row->drbl_volume; ?>" class="form-control" name="volume[]" placeholder="Volume" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drbl_satuan ?>" class="form-control" maxlength="50" name="satuan[]" placeholder="Satuan" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drbl_harga_satuan ?>" class="form-control inputHarga" name="harga_satuan[]" placeholder="Rp. 0" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drbl_sub_jumlah ?>" class="form-control inputHarga sub_jumlah" name="sub_jumlah[]" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
                  <td class="text-center"><a href="<?= site_url('rab/delete_blp_detil/'.$row->drbl_id.'/'.$id); ?>" id="btn-delete-detil" style="margin-bottom:16px;" type="button" class="btn btn-sm btn-danger" onclick="deleteRow(this)"><i class="fa fa-trash-o"></i></button></td>
                </tr>
                <?php $i++; ?>
              <?php endforeach; ?>
            <?php else: ?>
              ...
              <?php foreach($uraian as $row): ?>
                <tr>
                  ...
                  <td><div class="form-group"><input type="text" class="form-control" name="volume[]" placeholder="Volume" autocomplete="off"></div></td>
          ...
          <tfoot>
            <tr>
              ...
              <td><div class="form-group"><input type="text" value="<?= !empty($data['set']->blp_total) ? $data['set']->blp_total : set_value('total_jumlah'); ?>" class="form-control inputHarga" id="total_jumlah" name="total_jumlah" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/owner/blp/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->

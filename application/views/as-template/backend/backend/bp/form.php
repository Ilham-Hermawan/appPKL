...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->bp_id) ? $id = $data['set']->bp_id : $id = "0";
    ...
    ?>
    <?= form_open_multipart("dashboard/owner/bp/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
      ...
      <div class="box-body">
        ...
        <table style="width:100%" id="table">
          ...
          <tbody>
            <?php
              $uraian = array(
                'Biaya Brosur [A4]', 'Biaya Brosur [A3]', 'Biaya Roll Up Banner [0.6x1.6]', 'Biaya Baliho [2.4x22]',
                'Biaya Baliho [3.6x6]', 'Biaya V Banner [0.8x3]', 'Biaya Spanduk [1x5]', 'Biaya Umbul-umbul [1x5]',
                'Biaya Event Pameran', 'Biaya Desain Media Iklan', 'Biaya Pemasangan Media Iklan', 'Biaya Lain-lain'
              );
            ?>
            <?php $i = 1; ?>
            <?php if(!empty($data['detil'])): ?>
              <?php foreach($data['detil'] as $row): ?>
                <tr>
                  ...
                  <td><div class="form-group"><input type="text" value="<?= ($row->drbp_volume == 0) ? "" : $row->drbp_volume; ?>" class="form-control" name="volume[]" placeholder="Volume" autocomplete="off"></div></td>
                  ...
                  <td class="text-center"><a href="<?= site_url('rab/delete_bp_detil/'.$row->drbp_id.'/'.$id); ?>" id="btn-delete-detil" style="margin-bottom:16px;" type="button" class="btn btn-sm btn-danger" onclick="deleteRow(this)"><i class="fa fa-trash-o"></i></button></td>
                </tr>
                <?php $i++; ?>
              <?php endforeach; ?>
            <?php else: ?>
          ...
          <tfoot>
            <tr>
              ...
              <td><div class="form-group"><input type="text" value="<?= !empty($data['set']->bp_total) ? $data['set']->bp_total : set_value('total_jumlah'); ?>" class="form-control inputHarga" id="total_jumlah" name="total_jumlah" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/owner/bp/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->

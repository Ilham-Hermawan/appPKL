...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->bpt_id) ? $id = $data['set']->bpt_id : $id = "0";
    ...
    ?>
    <?= form_open_multipart("dashboard/owner/bpt/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
      ...
      <div class="box-body">
        ...
        <table style="width:100%" id="table">
          ...
          <tbody>
            <?php
              $uraian = array(
                'Harga Dasar Tanah', 'Biaya Pajak Perolehan Tanah', 'Biaya Lain-lain'
              );
            ?>
            <?php $i = 1; ?>
            <?php if(!empty($data['detil'])): ?>
              <?php foreach($data['detil'] as $row): ?>
                <tr>
                  ...
                  <td>
                    <div class="form-group"><input type="text" value="<?= $row->drb_uraian ?>" class="form-control" name="uraian[]" placeholder="Uraian" autocomplete="off"></div>
                    <input type="hidden" name="drb_id[]" value="<?= $row->drb_id ?>">
                  </td>
                  <td><div class="form-group"><input type="text" value="<?= ($row->drb_volume == 0) ? "" : $row->drb_volume; ?>" class="form-control" name="volume[]" placeholder="Volume" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drb_satuan ?>" class="form-control" maxlength="50" name="satuan[]" placeholder="Satuan" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drb_harga_satuan ?>" class="form-control inputHarga" name="harga_satuan[]" placeholder="Rp. 0" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drb_sub_jumlah ?>" class="form-control inputHarga sub_jumlah" name="sub_jumlah[]" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
                  <td class="text-center"><a href="<?= site_url('rab/delete_bpt_detil/'.$row->drb_id.'/'.$id); ?>" id="btn-delete-detil" style="margin-bottom:16px;" type="button" class="btn btn-sm btn-danger" onclick="deleteRow(this)"><i class="fa fa-trash-o"></i></button></td>
                </tr>
                <?php $i++; ?>
              <?php endforeach; ?>
            <?php else: ?>
          ...
          <tfoot>
            <tr>
              ...
              <td><div class="form-group"><input type="text" value="<?= !empty($data['set']->bpt_total) ? $data['set']->bpt_total : set_value('total_jumlah'); ?>" class="form-control inputHarga" id="total_jumlah" name="total_jumlah" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/owner/bpt/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->

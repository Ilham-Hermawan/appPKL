    ...
    <div class="box">
      ...
      <div class="box-body">
        <div style="margin-bottom:20px;">
          <div class="row">
            <div class="col-md-6">
              <a href="<?= site_url('dashboard/finance/transaksi/imb/add'); ?>" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Data</a>
            </div>
    ...
    $(window).load(function(){
      ...
      $(document).on("click","a[id='btn-hapus']", function (e) {
        ...
        function(isConfirm){
          if (isConfirm) {
            ...
            window.location.href = '<?= site_url('finance/imb_delete/'."'+id+'"); ?>';
          } else {
      ...
      function loadDt(perumahan){
        $('#data-table').DataTable({
          ...
          "serverSide": true,
          "bDestroy" : true,
          "ajax": "<?= site_url('finance/get_imb_list/'.'"+perumahan+"'); ?>",
          "sServerMethod": "POST",
          "language": {
          ...
          "order": [[ 0, "DESC" ]],
          "columns": [
            { "data": "imb_id", "width": "1%", name: "imb_id", "searchable": false, "sortable": false, className: 'text-center'},
            { "data": "imb_no", "sortable": true, "searchable": true, name: "imb_no", "width": "15%" },
            { "data": "imb_harga", "sortable": true, "searchable": true, name: "imb_harga", "width": "15%" },
            { "data": "rumah_nama", "sortable": true, "searchable": true, name: "rumah_nama" },
            { "data": "imb_date", "sortable": false, "searchable": false, name: "imb_date", "width": "3%" }
          ],
          "columnDefs": [
            {
              "render": function ( data, type, row ) {
                return '<div class="btn-group">'+
                       ...
                       '<li><a href="<?= site_url('dashboard/finance/transaksi/imb/dokument/'."'+row.imb_id+'"); ?>" target="_blank" title="Print" id="btn-print"><span class="fa fa-print"></span> Print</a></li>'+
                       '<li><a href="<?= site_url('dashboard/finance/transaksi/imb/edit/'."'+row.imb_id+'"); ?>" title="Edit" id="btn-edit"><span class="fa fa-pencil"></span> Edit</a></li>'+
                       '<li><a href="javascript:void(0)" data-id="'+row.imb_id+'" title="Hapus" id="btn-hapus"><span class="fa fa-trash-o text-red"></span> Hapus</a></li>'+
                       '</ul></div>';

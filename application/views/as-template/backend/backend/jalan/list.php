            ...
            <tr>
              ...
              <th>
                <select class="form-control" name="filterStatus" id="filterStatus" style="font-size:12px;width:70px !important">
                  ...
                  <option value="n">BELUM DIPROSES</option>
                </select>
              </th>
            </tr>
            <tr>
...
<script>
    $(window).load(function(){
      ...
      $('#view-modal').on('show.bs.modal', function(e) {
         ...
         $.ajax({
             type: "GET",
             url: "<?=site_url('finance/get_jalan_by_id/" + id + "');?>",
             cache: false,
      ...
      $(document).on("click","a[id='btn-sudah']", function (e) {
        ...
        function(isConfirm){
          if (isConfirm) {
            ...
            window.location.href = '<?= site_url('finance/change_j_status/'."'+id+'/'+status+'/'+booking_id+'"); ?>';
          } else {
      ...
      function loadDt(perumahan, status){
        $('#data-table').DataTable({
          ...
          "serverSide": true,
          "bDestroy" : true,
          "ajax": "<?= site_url('finance/get_jalan_list/'.'"+perumahan+"/"+status+"'); ?>",
          "sServerMethod": "POST",
          "language": {
          ...
          "order": [[ 0, "DESC" ]],
          "columns": [
            { "data": "j_id", "width": "8%", name: "j_id", "searchable": false, "sortable": false, className: 'text-center'},
            { "data": "booking_no", "sortable": true, "searchable": true, name: "booking_no", "width": "8%" },
            { "data": "j_harga", "sortable": true, "searchable": true, name: "j_harga", "width": "8%" },
            ...
            { "data": "j_status", "sortable": true, "searchable": true, name: "j_status", "width": "6%" },
            { "data": "j_datetime", "sortable": true, "searchable": true, name: "j_datetime","width": "18%" }
          ],
          "columnDefs": [
            {
              "render": function ( data, type, row ) {
                return '<div class="btn-group">'+
                       ...
                       '<li><a href="javascript:void(0)" data-id="'+row.j_id+'" status-id="y" booking-id="'+row.booking_id+'" title="Sudah" id="btn-sudah"><span class="fa fa-check text-green"></span> Sudah</a></li>'+
                       '<li><a href="javascript:void(0)" data-id="'+row.j_id+'" status-id="p" booking-id="'+row.booking_id+'" title="Proses" id="btn-proses"><span class="fa fa-spinner text-blue"></span> Proses</a></li>'+
                       '<li><a href="javascript:void(0)" data-id="'+row.j_id+'" status-id="n" booking-id="'+row.booking_id+'" title="Ditolak" id="btn-belum"><span class="fa fa-ban text-red"></span> Belum di Proses</a></li>'+
            ...
            },
            {
              "render": function ( data, type, row ) {
                if(data === "p"){
                  ...
                  return '<span class="label-text bg-red">BELUM DI PROSES</span>';
                }
              },

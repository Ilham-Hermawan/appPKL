    ...
    $(window).load(function(){
      ...
      $(document).on("click","a[id='btn-sudah']", function (e) {
        ...
        function(isConfirm){
          if (isConfirm) {
            ...
            window.location.href = '<?= site_url('transaksi/change_sp3k_status/'."'+id+'/'+status+'/'+booking_id+'"); ?>';
          } else {
      ...
      function loadDt(perumahan, status){
        $('#data-table').DataTable({
          ...
          "serverSide": true,
          "bDestroy" : true,
          "ajax": "<?= site_url('transaksi/get_sp3k_list/'.'"+perumahan+"/"+status+"'); ?>",
          "sServerMethod": "POST",
          "language": {
          ...
          "order": [[ 0, "DESC" ]],
          "columns": [
            { "data": "sp3k_id", "width": "8%", name: "sp3k_id", "searchable": false, "sortable": false, className: 'text-center'},
            ...
            { "data": "sp3k_status", "sortable": true, "searchable": true, name: "sp3k_status", "width": "6%" },
            { "data": "sp3k_datetime", "sortable": true, "searchable": true, name: "sp3k_datetime","width": "18%" }
          ],
          "columnDefs": [
            {
              "render": function ( data, type, row ) {
                return '<div class="btn-group">'+
                       ...
                       '<li><a href="javascript:void(0)" data-id="'+row.sp3k_id+'" status-id="y" booking-id="'+row.booking_id+'" title="Sudah" id="btn-sudah"><span class="fa fa-check text-green"></span> Sudah</a></li>'+
                       '<li><a href="javascript:void(0)" data-id="'+row.sp3k_id+'" status-id="p" booking-id="'+row.booking_id+'" title="Proses" id="btn-proses"><span class="fa fa-spinner text-blue"></span> Proses</a></li>'+
                       '<li><a href="javascript:void(0)" data-id="'+row.sp3k_id+'" status-id="n" booking-id="'+row.booking_id+'" title="Ditolak" id="btn-belum"><span class="fa fa-ban text-red"></span> Ditolak</a></li>'+
                       '</ul></div>';

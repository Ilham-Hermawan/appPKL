...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->wawancara_id) ? $id = $data['set']->wawancara_id : $id = "0";
    ...
    ?>
    <?= form_open("dashboard/admin/transaksi/wawancara/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
              ...
              <div class="form-group">
                <?php
                  $atrNo = array(
                    ...
                    'autocomplete'=> 'off',
                    'required'    => 'required',
                    'value'       => !empty($data['wawancara_no']) ? $data['wawancara_no'] : $data['set']->wawancara_no
                  );
                  if(!empty($data['set']->wawancara_no)){
                    $atrNo['readonly'] = 'true';
                  }
                ?>
                <div class="row">
              ...
              <div class="form-group">
                <?php
                  $atrPerihal = array(
                    ...
                    'name'        => 'inputPerihal',
                    'class'       => 'form-control',
                    'placeholder' => 'Perihal Wawancara',
                    'autocomplete'=> 'off',
                    'rows'        => '2',
                    'required'    => 'required',
                    'value'       => !empty($data['set']->wawancara_perihal) ? $data['set']->wawancara_perihal : set_value('inputPerihal')
                  );
                ?>
                <div class="row">
                  <div class="col-sm-12">
                    <?= form_label('Perihal Wawancara', 'inputPerihal'); ?>
                    <?= form_textarea($atrPerihal); ?>
                    <?php if(form_error('inputPerihal')){ echo form_error('inputPerihal', '<div class="text-danger">', '</div>'); }?>
                  </div>
                </div>
              </div><!--end form-group-->
              <div class="form-group">
                <?php
                  $atrTgl = array(
                    ...
                    'autocomplete'=> 'off',
                    'required'    => 'required',
                    'value'       => !empty($data['set']->wawancara_tanggal) ? $data['set']->wawancara_tanggal : date('Y-m-d')
                  );
                ?>
                <div class="row">
                  <div class="col-sm-12">
                  ...
                  <?php foreach($data['detil'] as $row): ?>
                    <tr>
                      <td>
                        ...
                        <input type="hidden" id="IDDWawancara" name="IDDWawancara[]" value="<?= $row->dw_id ?>">
                      </td>
                      <td>
                      ...
                      <td>
                        <a class="btn btn-danger" href="<?= site_url('transaksi/detil_wawancara_delete/'.$id.'/'.$row->dw_id); ?>" /><i class="fa fa-trash-o"></i></a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/admin/transaksi/wawancara/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->

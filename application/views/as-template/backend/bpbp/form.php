...
<div class="content-wrapper">
  ...
  <section class="content-header content-header-default">
    ...
    <ol class="breadcrumb">
      ...
      <li>Biaya Pajak & Bunga Pinjaman</li>
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->bpbp_id) ? $id = $data['set']->bpbp_id : $id = "0";
    ...
    ?>
    <?= form_open_multipart("dashboard/owner/bpbp/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
          ...
          <div class="form-group">
            ...
            <div class="row">
              <div class="col-sm-12">
                ...
                <?= form_dropdown('inputRab', $inputRab, $rab_id, 'class="form-control input-lg select2"'); ?>
        ...
        <table style="width:100%" id="table">
            ...
            <?php if(!empty($data['detil'])): ?>
              <?php foreach($data['detil'] as $row): ?>
                <tr>
                  ...
                  <td>
                    <div class="form-group"><input type="text" value="<?= $row->drbp_uraian ?>" class="form-control" name="uraian[]" placeholder="Uraian" autocomplete="off"></div>
                    <input type="hidden" name="drbp_id[]" value="<?= $row->drbp_id ?>">
                  </td>
                  <td><div class="form-group"><input type="text" value="<?= ($row->drbp_volume == 0) ? "" : $row->drbp_volume; ?>" class="form-control volume" name="volume[]" placeholder="Volume" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drbp_satuan ?>" class="form-control" maxlength="50" name="satuan[]" placeholder="Satuan" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drbp_harga_satuan ?>" class="form-control inputHarga" name="harga_satuan[]" placeholder="Rp. 0" autocomplete="off"></div></td>
                  <td><div class="form-group"><input type="text" value="<?= $row->drbp_sub_jumlah ?>" class="form-control inputHarga sub_jumlah" name="sub_jumlah[]" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
                  <td class="text-center"><a href="<?= site_url('rab/delete_bpbp_detil/'.$row->drbp_id.'/'.$id); ?>" id="btn-delete-detil" style="margin-bottom:16px;" type="button" class="btn btn-sm btn-danger" onclick="deleteRow(this)"><i class="fa fa-trash-o"></i></button></td>
                </tr>
                <?php $i++; ?>
              <?php endforeach; ?>
            <?php else: ?>
          ...
          <tfoot>
            <tr>
              ...
              <td><div class="form-group"><input type="text" value="<?= !empty($data['set']->bpbp_total) ? $data['set']->bpbp_total : set_value('total_jumlah'); ?>" class="form-control inputHarga" id="total_jumlah" name="total_jumlah" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/owner/bpbp/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->
  ...
  $(window).load(function(){
    ...
    function hitung_volume(){
      $(".volume").on('input', function () {
         ...
         if(self.closest('div').closest('td').next().find('input').val() === "%"){
           var persen = self.val();
           var sb = (persen / 100).toFixed(3);
           sb = sb * harga;
           self.closest('div').closest('td').next().find('input').closest('div').closest('td').next().find('input').closest('div').closest('td').next().find('input').val(sb);
         }
         else{
    ...
    function hitung_harga(){
      $(".inputHarga").on('input', function () {
         ...
         if(self.closest('div').closest('td').prev().find('input').val() === "%"){
           var sb = (volume / 100).toFixed(3);
           sb = sb * self.unmask();
           self.closest('div').closest('td').next().find('input').val(sb);
         }
         else{

...
<div class="content-wrapper">
  ...
  <section class="content-header content-header-default">
    ...
    <ol class="breadcrumb">
      ...
      <li>Biaya Penerimaan Penjualan</li>
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->penerimaan_id) ? $id = $data['set']->penerimaan_id : $id = "0";
    ...
    ?>
    <?= form_open_multipart("dashboard/finance/penerimaan/hpp/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
      ...
      <div class="box-body">
              ...
              <div class="form-group">
                <?php
                  ...
                  if(!empty($data['no_kwitansi'])){
                    ...
                    $atrNo['value'] = $data['set']->penerimaan_no;
                  }
              ...
              <div class="form-group">
                <?php
                  $atrDari = array(
                    ...
                    'autocomplete' => 'off',
                    'maxlength' => '255',
                    'value' => !empty($data['set']->penerimaan_dari) ? $data['set']->penerimaan_dari : set_value('inputDari')
                  );
                ?>
                <div class="row">
                  <div class="col-sm-12">
                    <?= form_label('Dari', 'inputDari'); ?>
                    <?= form_input($atrDari); ?>
                    <?php if(form_error('inputDari')){ echo form_error('inputDari', '<div class="text-danger">', '</div>'); }?>
                  </div>
                </div>
              </div><!--end form-group-->
            </div><!--end col-md-6-->
        ...
        <?php
          $uraian = array(
            'Biaya Perolehan Tahan', 'Biaya Persiapan & Pengolahan Tanah',
            'Biaya Legalitas & Perijinan', 'Biaya Prasarana, Sarana & Utilitas', 'Biaya Konstruksi Rumah'
          );
        ?>
        ...
        <table style="width:100%" id="table">
            ...
            <?php if(!empty($data['detil'])): ?>
              <?php foreach($data['detil'] as $row): ?>
                <tr>
                  ...
                  <td class="text-center"><a href="<?= site_url('penerimaan/delete_hpp_detil/'.$row->dp_id.'/'.$id); ?>" id="btn-delete-detil" style="margin-bottom:16px;" type="button" class="btn btn-sm btn-danger" onclick="deleteRow(this)"><i class="fa fa-trash-o"></i></button></td>
                </tr>
                <?php $i++; ?>
              <?php endforeach; ?>
            <?php else: ?>
          ...
          <tfoot>
            <tr>
              ...
              <td><div class="form-group"><input type="text" value="<?= !empty($data['set']->penerimaan_total) ? $data['set']->penerimaan_total : set_value('total_jumlah'); ?>" class="form-control inputHarga" id="total_jumlah" name="total_jumlah" placeholder="Rp. 0" autocomplete="off" readonly="true"></div></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/finance/penerimaan/hpp/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->

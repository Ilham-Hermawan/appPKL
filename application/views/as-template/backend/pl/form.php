...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <?php
      !empty($data['set']->pl_id) ? $id = $data['set']->pl_id : $id = "0";
    ...
    ?>
    <?= form_open("dashboard/finance/transaksi/pl/save", $atrform); ?>
    <?= form_hidden('id', $id); ?>
    <div class="box">
              ...
              <div class="form-group">
                <?php
                  $atrNo = array(
                    ...
                    'autocomplete'=> 'off',
                    'required'    => 'required',
                    'value'       => !empty($data['pl_no']) ? $data['pl_no'] : $data['set']->pl_no
                  );
                  if(!empty($data['set']->pl_no)){
                    $atrNo['readonly'] = 'true';
                  }
                ?>
                <div class="row">
              ...
              <div class="form-group">
                <?php
                  $atrTgl = array(
                    ...
                    'autocomplete'=> 'off',
                    'required'    => 'required',
                    'value'       => !empty($data['set']->pl_darw) ? $data['set']->pl_darw : date('Y-m-d')
                  );
                ?>
                <div class="row">
                  <div class="col-sm-12">
                  ...
                  <div class="col-sm-12">
                    <?php
                      $atrHarga = array(
                        ...
                        'autocomplete'=> 'off',
                        'required'    => 'required',
                        'value'       => !empty($data['set']->pl_harga) ? $data['set']->pl_harga : set_value('inputHarga')
                      );
                    ?>
                    <div class="row">
                      <div class="col-sm-12">
                  ...
                  <?php foreach($data['detil'] as $row): ?>
                    <tr>
                      <td>
                        ...
                        <input type="hidden" id="IDDWawancara" name="IDDWawancara[]" value="<?= $row->dpl_id ?>">
                      </td>
                      <td>
                      ...
                      <td>
                        <a class="btn btn-danger" href="<?= site_url('finance/detil_pl_delete/'.$id.'/'.$row->dpl_id); ?>" /><i class="fa fa-trash-o"></i></a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
      ...
      <div class="box-footer">
        <div class="text-right">
          ...
          ?>
          <a href="<?php echo site_url('dashboard/finance/transaksi/pl/list'); ?>" title="Back" class="btn btn-default">Kembali</a>
          <?= form_submit($atrBtn); ?>
        </div>
      </div>
    </div><!-- /.box -->
